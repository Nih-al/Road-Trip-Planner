...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Auto-fade header bar after 3 seconds ────────────────
    const headerEl = document.querySelector('header.glass-panel');
    if (headerEl) {
        setTimeout(() => {
    ...
    // Load saved trips from backend on page load
    loadTrips();
    ...
    if (btnRefresh) btnRefresh.addEventListener('click', loadTrips);
});
...
async function handleTripPlan() {
    ...
    try {
        ...
        await saveTrip(startLoc, destLoc, trip.route.distance);
...
// ── Save trip to backend ─────────────────────────────────
async function saveTrip(source, destination, distance) {
    try {
        const token = localStorage.getItem("token");
        ...
        console.warn("Failed to save trip to server:", err.message);
    }
}
...
async function loadTrips() {
    ...
    try {
        ...
        if (emptyMsg) {
            emptyMsg.textContent = "Could not load trip history.";
            emptyMsg.style.display = "block";
        }
    }
}

// ── Render trips into the history panel ──────────────────
function renderTrips(trips) {
    ...
    if (!trips || trips.length === 0) {
        if (emptyMsg) {
            emptyMsg.textContent = "No trips yet. Plan your first road trip!";
            emptyMsg.style.display = "block";
        }
        return;
    }
    ...
    sorted.forEach((trip, idx) => {
        ...
        const distStr = trip.distance ? `${Math.round(trip.distance)} km` : '—';

        card.innerHTML = `
            <div class="history-route">
                <span>${trip.source || 'Unknown'}</span>
                <span class="history-arrow">→</span>
                <span>${trip.destination || 'Unknown'}</span>
            </div>
            <div class="history-meta">
                <span class="history-badge">${distStr}</span>
                <span>📅 ${dateStr}</span>
            </div>
        `;
